...
describe 'required files', :if => ['fedora', 'redhat'].include?(os[:family]) do
  ...
  describe file('/usr/lib/systemd/system/git-daemon.socket'), :if => os[:release] >= "7" do
    it { should be_file }
    it { should be_owned_by 'root' }
    it { should be_grouped_into 'root' }
  ...
  describe file('/usr/lib/systemd/system/git-daemon@.service'), :if => os[:release] >= "7" do
    it { should be_file }
    it { should be_owned_by 'root' }
    it { should be_grouped_into 'root' }
  ...
  describe file('/etc/init.d/git-daemon'), :if => os[:release] < "7" do
    it { should be_file }
    it { should be_owned_by 'root' }
    it { should be_grouped_into 'root' }
